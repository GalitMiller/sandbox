module Bricata
  module Plugins
    class OpenFPC
      ...
      include Bricata::Plugins::Helpers
      ...
      def build_url_parameters
        ...
        if @params.has_key?(:protocol)
          @params.merge!(:filename => "bricata-#{@params[:protocol]}-#{@event.ip.ip_src.to_i}#{@event.ip.ip_dst.to_i}")
        else
          @params.merge!(:filename => "bricata-#{@event.ip.ip_src.to_i}#{@event.ip.ip_dst.to_i}")
        end
