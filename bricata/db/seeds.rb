...
SnortSchema.create(:vseq => 107, :ctime => Time.now, :version => "Bricata #{Bricata::VERSION}") if SnortSchema.first.blank?

# Default user setup
User.create(:name => 'Administrator', :email => 'bricata@bricata.com', :password => 'bricata', :password_confirmation => 'bricata', :admin => true) if User.all.blank?

# Default Lookup setup
Lookup.create(:title => 'Reverse DNS Lookup', :value => 'http://api.hackertarget.com/reversedns/?q=${ip}')
Lookup.create(:title => 'GeoIP Info', :value => 'http://api.hackertarget.com/geoip/?q=${ip}')

# Bricata General Settings
Setting.set(:company, 'Bricata.com') unless Setting.company?
Setting.set(:email, 'bricata@bricata.com') unless Setting.email?
Setting.set(:signature_lookup, 'http://doc.emergingthreats.net/bin/view/Main/$$sid$$') unless Setting.signature_lookup?
...
# Validate Bricata Indexes
require "./lib/bricata/jobs/cache_helper"
include Bricata::Jobs::CacheHelper
