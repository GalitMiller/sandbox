module ApplicationHelper
  ...
  def title(header, title=nil, menu_content=true, &block)
    ...
    title_header = content_tag(:div, header, :id => 'title-header', :class => 'row12')
    
    if block_given?
      data = capture(&block).gsub("\n|\t", '')

      if menu_content
        menu_holder = content_tag(:ul, "#{capture(&block)}".html_safe, :id => 'title-menu') unless data.blank?
    ...
    return content_tag(:div, html, :id => 'title2')
  end
  ...
  def snorby_box(title, normal_size=true, &block)
    ...
    content_tag(:div, html, :id => 'bricata-box', :class => 'bricata-box')
  end
  ...
  def worker_status(show_image=false)
    ...
    validations = [{:check => Bricata::Jobs.sensor_cache?, :enabled => true, :desc => "Sensor Cache Job"},
                   {:check=> Bricata::Jobs.geoip_update?, :enabled => Setting.geoip?, :desc => "GeoIP Update Job"}
                  ]
    ...
    html = <<-EOF
      ...
      original-title="copied!" title="copy to clipboard">
        <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" 
                ...
                class="clippy" 
                id="clippy">
        <param name="movie" value="#{Bricata::CONFIG[:baseuri]}/flash/clippy.swf">
        ...
        <embed src="#{Bricata::CONFIG[:baseuri]}/flash/clippy.swf"
               width="14" 
               height="14" 
               name="clippy" 
               quality="high" 
