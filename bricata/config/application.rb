...
module Bricata
  ...
  unless RUBY_VERSION.match(/^1.9/)
    puts "Bricata requires Ruby version 1.9.x"
    puts "We suggest using Ruby Version Manager (RVM) https://rvm.io/ to install the newest release"
    exit 1
  end

  # Check For bricata_config.yml
  unless File.exists?("config/bricata_config.yml")
    puts "Bricata Configuration Error"
    puts "* Please EDIT and rename config/bricata_config.yml.example to config/bricata_config.yml"
    exit 1
  end
  ...
  # Bricata Environment Specific Configurations
  raw_config = File.read("config/bricata_config.yml")
  ...
  class Application < Rails::Application
    ...
    PDFKit.configure do |config|
      config.wkhtmltopdf = Bricata::CONFIG[:wkhtmltopdf]
    ...
    unless time_zone
      ...
      if detected_time_zone
        ...
        puts "No time_zone specified in bricata_config.yml; detected time_zone: #{time_zone}"
      else
        puts "*** Warning:  no time zone is set in config/application.rb. Using UTC as the default time and behavior may be unexpected."
        puts "*** You can manually set the timezone in config/bricata_config.yml in the time_zone setting."
    ...
    if CONFIG.has_key?(:ssl) && CONFIG[:ssl]
      ...
      config.action_mailer.default_url_options = { :protocol => 'https', :host => Bricata::CONFIG[:domain] }
    else
      config.action_mailer.default_url_options = { :host => Bricata::CONFIG[:domain] }
    end
  end
