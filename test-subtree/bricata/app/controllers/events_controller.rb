class EventsController < ApplicationController
  ...
  def email
    Delayed::Job.enqueue(Bricata::Jobs::EventMailerJob.new(params[:sid],
    params[:cid], params[:email], params[:hosturl]))
  ...
  def mass_action
    ...
    if options.empty?
      ...
      sql = Bricata::Search.build("true", true, options)
      ids = Event.get_collection_id_string(sql)

      if params[:jobqueue]
        Delayed::Job.enqueue(Bricata::Jobs::MassClassification.new(ids, params[:classification_id], User.current_user.id))
      else
  ...
  def lookup
    if Setting.lookups?
      @lookup = Bricata::Lookup.new(params[:address])
      render :layout => false
    else
