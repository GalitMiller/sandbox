class SensorsController < ApplicationController
  ...
  def destroy
  	...
  	unless Bricata::Worker.running?
	   redirect_to :back, :flash => { :error => "The bricata worker service must be running in order to delete a sensor." }
	  end
  	if @sensor.present?
  		@sensor.update(:pending_delete => true)
  		Delayed::Job.enqueue(Bricata::Jobs::SensorDeleteJob.new(@sensor.sid), :priority => 1) if @sensor.save
