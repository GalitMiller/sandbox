class PageController < ApplicationController
  ...
  def dashboard
    ...
    @classifications = Classification.all(:limit => 5, :order => [:events_count.desc])
    ...
    sigs = latest_ten_distinct_signatures
    ...
    respond_to do |format|
      ...
      format.js
      format.pdf do
        render :pdf => "Bricata Report - #{@start_time.strftime('%A-%B-%d-%Y-%I-%M-%p')} - #{@end_time.strftime('%A-%B-%d-%Y-%I-%M-%p')}", :template => "page/dashboard.pdf.erb", :layout => 'pdf.html.erb', :stylesheets => ["pdf"]
      end
    end
  ...
  def search
    @json = Bricata::Search.json
  end

  def search_json
    render :json => Bricata::Search.json
  end

  def force_cache
    Bricata::Jobs.force_sensor_cache
    render :json => {
      :caching => Bricata::Jobs.caching?,
      :problems => Bricata::Worker.problems?,
      :running => Bricata::Worker.running?,
      :daily_cache => Bricata::Jobs.daily_cache?,
      :sensor_cache => Bricata::Jobs.sensor_cache?
    }
  end
