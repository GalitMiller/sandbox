require 'netaddr'
require 'bricata/model/counter'
require 'bricata/pager'

class Event

  include DataMapper::Resource
  include Bricata::Model::Counter
  ...
  def self.sorty(params={}, sql=false, count=false)
    ...
    if params.has_key?(:search)
      sql, count = Bricata::Search.build(params[:match_all], false, params[:search])
  ...
  def packet_capture(params={})
    ...
    when :openfpc
      Bricata::Plugins::OpenFPC.new(self,params).to_s
    when :solera
      Bricata::Plugins::Solera.new(self,params).to_s
    else
      nil
    end
  end
  ...
  def send_notification
    Delayed::Job.enqueue(Bricata::Jobs::AlertNotifications.new(self.sid, self.cid))
  end
  ...
  def in_xml
    ...
    %{<bricata>#{to_xml}#{ip.to_xml}#{protocol_data.last.to_xml if protocol_data}#{classification.to_xml if classification}#{payload.to_xml if payload}</bricata>}.chomp
  end
